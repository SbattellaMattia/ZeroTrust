...
|`Blocked-host`                |-| Container esterno, bloccato dal firewall in `pep-envoy`.  |`external_net` |Per raggiungere l'esterno passa per il `pep-envoy`| 
|`Allowed-server`                |-| Simula un server raggiungibile dagli host interni.  |`external_net` |-| 
...
|`PDP` (OPA)          |<img width="75" height="75" alt="image" src="https://github.com/user-attachments/assets/92d2a85b-19ce-4df2-9744-d5a2fa267173" />| Policy Decision Point: valuta le policy basate su input (identità, risorsa, azione, trust score) e risponde ALLOW/DENY.    |  `internal_net` | Interroga il Trust Service per ottenere il trust score dinamico.                |
|`Trust Service`      |-| Calcola e ritorna il trust score dinamico.|`internal_net` | Fornisce input numerico/qualitativo al PDP basato sulla storia dell'utente registrata in Splunk. Aggiunto per separare le operazioni di calcolo e interazione con Splunk dal resto.|
...
|`Splunk`             |<img width="618" height="182" alt="image" src="https://github.com/user-attachments/assets/c6a2c0bb-0222-4c2d-adaf-35ada00cad95" /> | SIEM / collector di log centralizzato   | `management_net`, `internal_net`| I container inviano i log su `management_net`; Splunk analizza e visualizza eventi.|
